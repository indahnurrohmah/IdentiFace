...
const getTodaySessions = async (req, res, next) => {
    try {
        ...
        if (!lecturer) {
            return res.status(404).json({ success: false, message: 'Profil dosen tidak ditemukan.', error: null });
        }

        // 2. Fetch sessions via classRepository
        const sessions = await classRepository.getTodaySessionsByLecturer(lecturer.id_dosen);
...
const toggleSessionStatus = async (req, res, next) => {
    try {
        ...
        if (typeof is_active !== 'boolean') {
            return res.status(400).json({ success: false, message: 'Status is_active harus berupa boolean.', error: null });
        }

        const updatedSession = await classRepository.toggleSessionStatus(id_sesi, is_active);

        if (!updatedSession) {
            return res.status(404).json({ success: false, message: 'Sesi kelas tidak ditemukan.', error: null });
        }

        res.json({
            success: true,
            message: is_active ? 'Sesi kelas berhasil dimulai! Mahasiswa sekarang dapat melakukan presensi.' : 'Sesi kelas telah ditutup.',
            data: updatedSession
        });
    } catch (error) {
        next(error);
...
const getLiveAttendance = async (req, res, next) => {
    try {
        ...
        const isAuthorized = await classRepository.verifySessionLecturer(id_sesi, id_akun);
        if (!isAuthorized) {
            return res.status(403).json({ success: false, message: 'Akses ditolak. Anda tidak memiliki akses ke sesi kelas ini.', error: null });
        }

        // 2. Fetch live attendance data
        const attendanceList = await attendanceRepository.getLiveAttendanceBySesi(id_sesi);
...
const getDashboardStats = async (req, res, next) => {
    try {
        const id_akun = req.user.id_akun;
        const lecturer = await lecturerRepository.findByAccountId(id_akun);
        if (!lecturer) return res.status(404).json({ success: false, message: 'Profil dosen tidak ditemukan.' });

        const stats = await lecturerRepository.getDashboardSummary(lecturer.id_dosen);
